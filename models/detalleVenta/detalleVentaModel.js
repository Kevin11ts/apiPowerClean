...
class DetalleVenta {
    ...
    static async create(data) {
        ...
        try {
            const result = await pool.query(
                'INSERT INTO detalle_ventas (FolioVenta, IdProducto, Cantidad, created_at) VALUES ($1, $2, $3, current_timestamp) RETURNING *', [FolioVenta, IdProducto, Cantidad]
            );
    ...
    static async update(id, data) {
        ...
        try {
            const result = await pool.query(
                'UPDATE detalle_ventas SET FolioVenta = $1, IdProducto = $2, Cantidad = $3 WHERE IdDetalleVenta = $4 RETURNING *', [FolioVenta, IdProducto, Cantidad, id]
            );
            return result.rows[0];
        } catch (error) {
    ...
    static async searchAllColumns(searchString) {
        try {
            const result = await pool.query(
                ...
                CAST(Cantidad AS TEXT) ILIKE $1`, [`%${searchString}%`]
            );
            return result.rows;
        } catch (error) {
